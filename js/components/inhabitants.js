...
export class InhabitantsComponent {
  ...
  bindEvents() {
    const inhabitantInputs =
      this.container.querySelectorAll(".inhabitant-input");
  ...
  onInhabitantChange(event) {
    ...
    const houses =
      inhabitants > 0 ? Math.ceil(inhabitants / inhabitantData.ratio) : 0;
  ...
  onHouseChange(event) {
    ...
    inhabitantInput.removeEventListener(
      "input",
      this.onInhabitantChange.bind(this),
    );
    ...
    setTimeout(() => {
      inhabitantInput.addEventListener(
        "input",
        this.onInhabitantChange.bind(this),
      );
    }, 10);
