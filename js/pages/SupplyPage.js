...
export class SupplyPage extends BasePage {
    ...
    calculateConsumptionMatrix(inhabitants, goods) {
        ...
        inhabitants.forEach(inhabitant => {
            ...
            goods.forEach((good, goodIndex) => {
                const consumption = this.getConsumptionForInhabitantAndGood(inhabitant, goodIndex);
                if (consumption) {
    ...
    exportMatrix() {
        ...
        try {
            let csv = 'Population,' + this.supplyMatrix.goods.map(g => this.formatGoodTitle(g.title)).join(',') + '\n';
            ...
            this.showNotification('Failed to export matrix: ' + error.message, 'error');
        }
    }
