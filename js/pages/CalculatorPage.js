...
export class CalculatorPage extends BasePage {
  ...
  bindEvents() {
    ...
    if (memorizeBtn) {
      this.addEventListener(
        memorizeBtn,
        "click",
        this.memorizeAmounts.bind(this),
      );
    }
  ...
  calculate() {
    try {
      ...
      if (inhabitants.every((val) => val === 0)) {
        this.reset();
        this.showNotification(
          "Please set at least some population to calculate production needs.",
          "warning",
        );
        return;
      }
      ...
      const needs =
        this.calculationService.calculateProductionNeeds(inhabitants);
      ...
      this.table.updateLupeDisplay(
        ceiledNeeds,
        this.showProductionChain.bind(this),
      );
  ...
  memorizeAmounts() {
    if (!this.isCalculated) {
      this.showNotification(
        "Please calculate production needs first",
        "warning",
      );
      return;
    }
    ...
    this.showNotification(
      "Current production requirements memorized for comparison",
      "success",
    );
  }
  ...
  compareAmounts() {
    ...
    if (this.memorizedAmounts.length === 0) {
      this.showNotification(
        "Please memorize a configuration first using the Memorize button",
        "warning",
      );
      return;
    }
    ...
    if (this.table) {
      this.table.updateDifferenceDisplay(
        this.currentAmounts,
        this.memorizedAmounts,
      );
      this.showNotification(
        "Comparison with memorized configuration displayed",
        "info",
      );
    }
  }
  ...
  async loadFromXML() {
    try {
      ...
      if (
        ...
        data.calculationResults.some((val) => val > 0)
      ) {
  ...
  updateDisplayFromCalculatedAmounts() {
    ...
    try {
      ...
      console.error(
        "[CalculatorPage] Error updating display from calculated amounts:",
        error,
      );
    }
  }
