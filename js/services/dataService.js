class DataService {
  ...
  evaluateCondition(condition, inhabitants) {
    ...
    switch (condition.type) {
      case 'AND':
        return condition.conditions.every(cond => this.evaluateSingleCondition(cond, inhabitants));
      
      case 'OR':
        return condition.conditions.some(cond => this.evaluateSingleCondition(cond, inhabitants));
  ...
  calculateAllRequirements(inhabitants) {
    ...
    return goods.map(good => this.calculateGoodRequirement(good.title, inhabitants));
  }
  ...
  createSupplyMatrix() {
    ...
    inhabitants.forEach(inhabitant => {
      ...
      goods.forEach(good => {
        ...
        if (req) {
          const consumers = req.consumers.filter(c => c.type === inhabitant.title);
