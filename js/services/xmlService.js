export class XMLService {
  ...
  importFromXML(xmlString) {
    try {
      ...
      if (root.tagName !== 'Anno1404Calculator') {
        throw new Error('Invalid file format. Expected Anno1404Calculator root element.');
      }
  ...
  getInhabitantType(index) {
    const types = ['beggar', 'peasant', 'citizen', 'patrician', 'nobleman', 'nomad', 'envoy'];
    return types[index] || 'unknown';
  }

  getGoodType(index) {
    const types = [
      'fish', 'spices', 'bread', 'meat', 'cider', 'beer', 'wine',
      'linen_garments', 'leather_jerkins', 'fur_coats', 'brocade_robes',
      'books', 'candlesticks', 'glasses', 'dates', 'milk', 'carpets',
      'coffee', 'pearl_necklaces', 'parfumes', 'marzipans'
    ];
    return types[index] || 'unknown';
  }
